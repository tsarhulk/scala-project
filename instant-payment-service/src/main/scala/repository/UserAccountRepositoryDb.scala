...
class UserAccountRepositoryDb(implicit val ec: ExecutionContext, db: Database) extends UserAccountRepository {
            ...
            updateMoney = startMoney
                .map { startMoney =>
                    if (startMoney < buffer)
                        ...
                        Right(startMoney + buffer)
                }
                .getOrElse(Left("Элемент не найден"))

            future = updateMoney.map(money => db.run { query.update(money) }) match {
    ...
    override def moneyTransfer(transaction: MoneyTransfer): Future[Either[String, ChangeMoneyResult]] = {
        ...
        for {
            ...
            // кешбэк
            cashbackMoney = (buffer.toDouble * categoryCashback.getOrElse(transaction.category, defaultCashback)).toInt
            ...
            recipientUpd = recipientOpt
                .map { recipientMoney =>
                    {
                        Right(recipientMoney + buffer)
                    }
                }
                .getOrElse(Left("Элемент не найден"))

            senderFuture =
                senderUpd.map(money => db.run(sender.update(money))) match {
                    case Right(_) =>
                        recipientUpd.map(money => db.run(recipient.update(money))) match {
                            case Right(future) => future.map(Right(_))
                            case Left(s) => Future.successful(Left(s))
                        }
                    case Left(s) => Future.successful(Left(s))
