    ...
    def route: Route = {
        (path("users") & get) { // список всех пользователей
            ...
            path("user") { // обновляем номер телефона пользователя
                (put & entity(as[UpdatePhoneNumber])) { updateNumberPhone =>
            ...
            (put & path("user" / "money" / "refill") & entity(as[RefillMoney])) { refillMoney => // пополняем счёт
                onSuccess(repository.refillMoney(refillMoney)) {
                    case Right(value) => complete(value)
                    case Left(s) => complete(StatusCodes.NotAcceptable, s)
                }
            } ~
            (put & path("user" / "money" / "withdraw") & entity(as[WithdrawMoney])) { withdrawMoney => /* списываем
                 * деньги со счёта */
